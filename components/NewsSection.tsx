  ...
  return (
    <section className="px-margin-mobile md:px-margin-desktop max-w-container-max mx-auto py-24">
      <div className="flex justify-between items-end mb-12">
        <div>
          <h2 className="font-h2 text-h2 text-on-surface">Berita & Pengumuman</h2>
          <p className="font-body-md text-on-surface-variant">Dapatkan kabar terbaru dari Nganjuk Creative Hub.</p>
        </div>
      </div>
        ...
        {news.map((item, index) => (
          <div key={index} className="glass-card rounded-3xl overflow-hidden group hover:border-primary/40 transition-all flex flex-col sm:flex-row">
            <div className="w-full sm:w-1/3 h-48 sm:h-auto overflow-hidden">
                <Image src={item.img} alt={item.title} width={300} height={200} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" />
            </div>
